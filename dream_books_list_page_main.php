...
<body>
	<div id="out">
      ...
      //取得記錄數
		...
		if(mysql_result($result_2, 0, "author_banner_img_path") == ""){
      ...
      //將記錄指標移至本頁第一筆記錄的序號  
			...
			if(!mysql_data_seek($result, $started_record)){
	  ...
	  echo "<BR><center><font face='華康儷粗宋' color='#800080' size='70'>".$_GET["an"]."─夢之鄉作品集</font></center><BR><BR>";
      ...
      while ($row = mysql_fetch_assoc($result) and $j <= $records_per_page)
      {
        ...
        echo "第一章試閱：<a href='dream_books_list_page_main_text.php?ac=".$row["author_code"]."&bnc=".$row["book_name_code"]."' target='_blank'><img src='./images/author_01.gif' title='點我試閱' width='40' height='40'></a>";
        echo "</td>";
		echo "<td>";
		echo "</td>";
      ...
      if ($page > 1)
        echo "<a href='./dream_books_list_page_main.php?ac=".$_GET["ac"]."&an=".$_GET["an"]."&page=". ($page - 1) . "'>上一頁</a> ";
      ...
      for ($i = 1; $i <= $total_pages; $i++)
      {
        if ($i == $page)
		     ...
		     {
                 echo "<a href='./dream_books_list_page_main.php?ac=".$_GET["ac"]."&an=".$_GET["an"]."&page=".$i."'>&nbsp;".$i."</a>";
		     }
      }
			
      if ($page < $total_pages)
        echo "<a href='./dream_books_list_page_main.php?ac=".$_GET["ac"]."&an=".$_GET["an"]."&page=". ($page + 1) . "'>下一頁</a> ";			
	...
	<a style="display:scroll;position:fixed;bottom:5px;right:5px;" href="#" title="Back to Top"><img onmouseover="this.src='./images/go-go-top.png'" src="./images/go-go-top_1.png" onmouseout="this.src='./images/go-go-top_1.png'"/></a>
</body></html>