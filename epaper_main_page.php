...
<body>
	<div id="out">
		...
		<div id="middle_epaper_title_name_pic2">
		<?php 
	          $sel_sql = "SELECT * FROM $dbtable_epaper_history_list WHERE epaper_id = '".mysql_real_escape_string($_GET["e_id"])."'";
              $result = mysql_query($sel_sql) or die("失敗");
			  
		if(mysql_result($result, 0, "epaper_title") == "" or !is_numeric(mysql_real_escape_string($_GET["e_id"]))){
              ...
              echo mysql_result($result, 0, "epaper_title"); 
		?>
		</div>
		<div id="middle_epaper_text">
		<?php echo mysql_result($result, 0, "epaper_content"); ?>
		</div>
