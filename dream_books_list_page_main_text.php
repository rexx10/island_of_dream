...
<body>
	<div id="out">
	  ...
	  //$sel_sql = "SELECT * FROM $dbtable_author_book_list_goa WHERE author_code = '".mysql_real_escape_string($_GET["ac"])."'";
      $sel_sql = "SELECT * FROM author_book_test_read WHERE author_code = '".mysql_real_escape_string($_GET["ac"])."' AND 
	                                                        book_name_code = '".mysql_real_escape_string($_GET["bnc"])."'";
	  ...
	  $result_2 = mysql_query($sel_sql_2) or die("失敗");
		...
		if(mysql_result($result, 0, "book_content") == "" or mysql_result($result_2, 0, "author_banner_img_path") == ""){
      ...
      echo "<BR>《夢之鄉》".$_GET["bnc"];
	  echo "<BR><BR>";
	  echo "<center><img src='".mysql_result($result_2, 0, "author_banner_img_path")."'></center><BR>";
	  echo "<BR><BR>";
	  echo "<p style='text-align:left;'>". nl2br(mysql_result($result, 0, "book_content"))."</p>"; 
	  ?>
	  		</div>
		</div>
		</div>
